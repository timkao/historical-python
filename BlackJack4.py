...
class Start_Scene(object):
	...
	# check whether the player have enough money. if not, the game would not start.
		if Player1.money <= 0:
			...
			pass
	# check if there is enough cards in the card decks, if not, fill in one more set of cards. 
	...
	# reset the decks whenever restart the game.	
		self.additional_deck = []
		Player1.yourcarddeck = []
		Player1.yoursplitdeck = []
		Player2.bosscarddeck = []
		
	# the player starts from below:	
...
class Second_Scene(object):
	...
	# Exclude the situations in which the game is over without going to the opponent's turn
		if (sumcard(Player1.yourcarddeck) > 21 or len(Player1.yourcarddeck) == 5) and (sumcard(Player1.yoursplitdeck) > 21 or len(Player1.yoursplitdeck) == 5):
			...
			pass
	# If you split the card, you will start from below:	
		control = True
		while control:
			print "What would you want to do?"
